...
class SnakeGamePanel extends JPanel implements ActionListener {
    ...
    public void checkCollisions() {
        ...
        // check if head is to the left of the game area
        if (x[0] < 0) {
            x[0] = SCREEN_WIDTH - UNIT_SIZE;
        }

        // check if head is to the right of the game area
        if (x[0] >= SCREEN_WIDTH) {
            x[0] = 0;
        }

        // check if head is above the game area
        if (y[0] < 0) {
            y[0] = SCREEN_HEIGHT - UNIT_SIZE;
        }

        // check if head is below the game area
